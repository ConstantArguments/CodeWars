...
def tribonacci(signature, n):
    """
    Returns list starting with signature, continuing the pattern until
    list length is n.
    :param signature: list containing int
    :param n: int
    """
     # Sum last three in list. Append to list. Repeat until list length is n.
    while len(signature) < n:
        next = sum(signature[-3:])
        signature.append(next)
    # Specify range to also handle n < 3
    return signature[:n]
